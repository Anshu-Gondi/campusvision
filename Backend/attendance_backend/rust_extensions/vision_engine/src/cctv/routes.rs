...
pub async fn process_frame_route(
    ...
    let frame = match frame_bytes {
        Some(f) => f,
        None => return (StatusCode::BAD_REQUEST, "frame is required").into_response(),
    };

    let result = process_frame_rust(
        &frame,
        &q.role,
        &q.camera_id,
        q.min_confidence.unwrap_or(0.75),
        q.min_track_hits.unwrap_or(5),
        model_path.as_deref(),
    );
...
pub async fn get_tracks_route(
    Path((role, camera_id)): Path<(String, String)>,
) -> Json<Vec<CctvResult>> {
