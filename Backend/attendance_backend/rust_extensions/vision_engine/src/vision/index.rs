...
pub fn search_person_rust(
    ...
    Ok(embeddings::search_in_role(&embedding, &role, k))
}
...
pub fn can_reenroll_rust(
    ...
    embeddings::can_reenroll(&embedding, person_id, &role)
        .map_err(|e| e.to_string())
}
...
pub fn query_similar_rust(
    ...
    embeddings::search_in_role(&embedding, "student", k)
        .into_iter()
        .map(|(id, _)| id)
        .collect()
}
