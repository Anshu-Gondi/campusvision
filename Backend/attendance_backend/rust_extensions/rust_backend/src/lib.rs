...
use crate::scheduler::scheduler::{ClassRequest, FullScheduler};
use cctv_state::*;
...
pub fn schedule_classes(py_classes: Vec<PyObject>) -> PyResult<PyObject> {
    Python::with_gil(|py| {
        ...
        let mut scheduler = FullScheduler::new();
        let results = scheduler.assign_classes(&classes);

        // Convert Rust output back to Python
        ...
        for r in results {
            ...
            d.set_item("score", r.teacher.similarity)?;
            py_list.append(d)?;
        }
